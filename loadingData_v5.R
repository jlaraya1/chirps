...
#18/9/2019
# Al calcular prec2 hice un redondeo de las coordenadas (latitud y longitud). He decidido retener la precisión del archivo
# original porque al hacer los gráficos de los grupos en otro experimento noto algo extraño en la distribución de las estaciones
# cercanas, y creo que el problema fue el redondeo
...
print("REVISAR QUE EN LA CARPETA EXISTAN SOLO ARCHIVOS NETCDF, HAY OTROS DOS ARCHIVOS PARA EL MAPA Y LAS COORDENADAS QUE SE GUARDAN AQUI")
...
#for(i in seq(1,2)) { 
                                ...
                                prec2=as.data.frame(cbind(lon=lonRange[lo],lat=latRange[la],day=format(date,"%d"),month=format(date,"%m"),year=format(date,"%Y"),prec))
                                ...
                                write.table(prec2,quote=FALSE,file=paste(saveResultsHere,"precDia_CHIRPS_lon_",lonRange[lo],"_lat_",latRange[la],"_year_",year,".txt",sep=""),row.names=T,col.names=T)                                                                                                         
                                }}
 ...
 write.table(coordAll,quote=FALSE,file=paste(saveResultsHere,"coordenadas_archivos.txt",sep=""),row.names=T,col.names=T)                                                             
